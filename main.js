...
function ingresarDatosSeguro() {
  while (true) {
    ...
    const precioVehiculo = Number(
      prompt("Ingrese el precio estimado del vehículo:")
    );
    ...
    const cantidadCuotas = parseInt(
      prompt("Ingrese la cantidad de meses de contrato:")
    );

    if (isNaN(cantidadCuotas)) {
      mostrarMensaje(
        "Por favor, ingrese un valor numérico válido para la cantidad de meses de contrato."
      );
      return;
    }

    const cuotaSeguro = calcularCuotaSeguro(
      precioVehiculo,
      cantidadCuotas,
      opcion
    );
    ...
    mostrarMensaje(
      `La cuota mensual para el vehículo ${nombreVehiculo} es: ${cuotaSeguro}`
    );
  }
}
